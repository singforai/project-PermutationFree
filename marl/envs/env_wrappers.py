...
def shareworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        ...
        if cmd == "step":
            ob, s_ob, reward, done, total_dones, info, available_actions, visiable_objects = env.step(data)
            if "bool" in done.__class__.__name__:  # done is a bool
                if (
                    ...
                    ob, s_ob, available_actions, visiable_objects = env.reset()
            else:
                if np.all(
                    done
                ):  # if done, save the original obs, state, and available actions in info, and then reset
            ...
            remote.send((ob, s_ob, reward, done, total_dones, info, available_actions, visiable_objects))
        elif cmd == "reset":
            ob, s_ob, available_actions, visiable_objects = env.reset()
            remote.send((ob, s_ob, available_actions, visiable_objects))
        elif cmd == "reset_task":
            ob = env.reset_task()
            remote.send(ob)
        elif cmd == "render":
...
class ShareSubprocVecEnv(ShareVecEnv):
    ...
    def step_wait(self):
        ...
        self.waiting = False
        obs, share_obs, rews, dones, total_dones, infos, available_actions, visiable_objects = zip(*results)
        return (
            np.stack(obs),
            np.stack(share_obs),
            np.stack(rews),
    ...
    def reset(self):
        ...
        results = [remote.recv() for remote in self.remotes]
        obs, share_obs, available_actions, visiable_objects = zip(*results)
        return np.stack(obs), np.stack(share_obs), np.stack(available_actions), np.stack(visiable_objects)
