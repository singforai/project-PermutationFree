from .distributions import Bernoulli, Categorical, DiagGaussian, AttentionCategorical
import torch
...
class ACTLayer(nn.Module):
    ...
    def action_prob(self, x, available_actions=None, deterministic=False):
        ...
        return actions, action_logits.probs
...
class ActLayer(nn.Module):
    """
    Attention Module to compute actions.
    :param action_space: (gym.Space) action space.
    :param inputs_dim: (int) dimension of network input.
    :param use_orthogonal: (bool) whether to use orthogonal initialization.
    :param gain: (float) gain of the output layer of the network.
    ...
    def __init__(self):
        super().__init__()
        self.action_out = AttentionCategorical()
    
    def forward(self, output, available_actions=None, deterministic=False):
