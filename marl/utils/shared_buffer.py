...
class SharedReplayBuffer(object):
    ...
    def forward_transformer_generator(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        for indices in sampler:
            ...
            obs_batch = obs_batch.reshape(mini_batch_size * self.num_agents, obs_batch.shape[-1])
            print(np.array(obs_batch).shape)
            share_obs_batch = share_obs_batch.reshape(mini_batch_size * self.num_agents, share_obs_batch.shape[-1]) 
            rnn_states_batch = rnn_states_batch.reshape(mini_batch_size * self.num_agents, self.recurrent_N, self.hidden_size)
            ...
            masks_batch = masks_batch.reshape(mini_batch_size * self.num_agents, 1)
            ...
            yield obs_batch, share_obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch,\
                  value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch,\
                  adv_targ, available_actions_batch
    ...
    def recurrent_transformer_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        obs = _cast_v3(self.obs[:-1])     
        share_obs = _cast_v3(self.share_obs[:-1])
