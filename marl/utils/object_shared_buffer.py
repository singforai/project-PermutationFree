...
class ObjectSharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, num_object, obs_space, cent_obs_space, act_space):
        ...
        self.obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, self.num_object, *obs_shape), dtype=np.float32)

        self.rnn_states = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, self.num_object, self.recurrent_N, self.hidden_size),
            dtype=np.float32)
        ...
        self.masks = np.ones((self.episode_length + 1, self.n_rollout_threads, self.num_object, 1), dtype=np.float32)
    ...
    def feed_forward_generator_transformer(self, advantages, num_mini_batch=None, mini_batch_size=None): #mini-batch 1
        """
        ...
        """
        episode_length, n_rollout_threads, num_agents = self.rewards.shape[0:3] # 200 32 3
        batch_size = n_rollout_threads * episode_length # 6400
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        """
        episode_length, n_rollout_threads, num_agents = self.rewards.shape[0:3] # 200 32 3
        batch_size = n_rollout_threads * episode_length * num_agents # 19200
        data_chunks = batch_size // data_chunk_length  # [C=r*T*M/L] 19200 // 10
        mini_batch_size = data_chunks // num_mini_batch # 1920 // 1

        rand = torch.randperm(data_chunks).numpy() # (4000,) <= step을 마구 섞음. 
        sampler = [rand[i * mini_batch_size : (i + 1) * mini_batch_size] for i in range(num_mini_batch)] # (1, 4000)
        ...
        if len(self.share_obs.shape) > 4:
            ...
            share_obs = _cast(self.share_obs[:-1]) # share_obs shape: (40000, 393)
            obs = _cast(self.obs[:-1]) # obs shape: (40000, 334)
        actions = _cast(self.actions) # self.actions: (200, 20, 10, 1) => (40000, 1)
        ...
        rnn_states = self.rnn_states[:-1].transpose(1, 2, 0, 3, 4).reshape(-1, *self.rnn_states.shape[3:]) # (40000, 1, 64)
        ...
        for indices in sampler:
            ...
            share_obs_batch = _flatten(L, N, share_obs_batch) #(10, 4000, 393) => (40000, 393)
    ...
    def jrpo_recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        critic_obs = _cast_v3(self.share_obs[:-1])
        ...
        rnn_states_critic = (
            self.rnn_states_critic[:-1].transpose(1, 0, 2, 3, 4).reshape(-1, *self.rnn_states_critic.shape[2:])
        )
        ...
        for indices in sampler:
            ...
            for index in indices:
                ...
                # size [T+1 N M Dim]-->[T N M Dim]-->[N,M,T,Dim]-->[N*M*T,Dim]-->[L,Dim]
                # [L, agent_num, Dim]
                share_obs_batch.append(critic_obs[ind : ind + data_chunk_length])
            ...
            rnn_states_batch = np.stack(rnn_states_batch).reshape(N * self.num_agents, *self.rnn_states.shape[3:])
            rnn_states_critic_batch = np.stack(rnn_states_critic_batch).reshape(
                N * self.num_agents, *self.rnn_states_critic.shape[3:]
            )
            # Flatten the (L, N, ...) from_numpys to (L * N, ...)

            share_obs_batch = _flatten_v3(L, N, self.num_agents, share_obs_batch)
            obs_batch = _flatten_v3(L, N, self.num_agents, obs_batch)
            ...
            masks_batch = _flatten_v3(L, N, self.num_agents, masks_batch)
